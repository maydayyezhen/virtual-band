...
/** Sequential fixed-step simulation. This first exporter supports the authored Queen Canvas show. */
export class OfflineStage {
  ...
  begin(): { duration: number; width: number; height: number } {
    ...
    if (!player || !p.cameraShow.available) throw new Error('请先加载带导播编排的波西米亚示例');
