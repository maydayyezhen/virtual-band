 ...
 *
 * Instrument trims establish ensemble headroom. Program trims only compensate
 * systematic preset loudness jumps inside one instrument family. User volume,
 * MIDI expression and performance dynamics stay outside this table.
 */
...
export function mixTrimDb(
  ...
  const instrument = profile.instrumentTrimDb[target] ?? 0;
  if (program === undefined) return instrument;

  let programTrim = 0;
  if (target === 'acoustic') programTrim = profile.programTrimDb.acoustic[program] ?? 0;
  else if (target === 'electric') programTrim = profile.programTrimDb.electric[program] ?? 0;
  return instrument + programTrim;
}
