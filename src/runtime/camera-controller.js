...
// its private state, but every actual render is composed through this controller.
(() => {
  const T = THREE;
  ...
  const rootLabel = (root) => {
    ...
    if (electric) return '电吉他 ' + electric[1];
  ...
  // Capture all instrument roots. stage-layout.js and shadow-sync.js wrap Scene.add
  // after this file; both restore back to this wrapper when their startup work ends.
  const sceneAdd = T.Scene.prototype.add;
  T.Scene.prototype.add = function (...objects) {
    const result = sceneAdd.apply(this, objects);
    for (const object of objects) {
      if (isBandRoot(object) && !roots.includes(object)) roots.push(object);
    }
    return result;
  };

  // Capture the actual renderer/scene/camera and make Camera v2 authoritative at draw
  // time. This avoids invasive edits inside the large legacy app.js camera block.
  ...
  // Perspective fit using camera-space extents of every Box3 corner. Unlike the old
  // width/height constants this automatically adapts to song visibility, stage edits,
  // aspect ratio and imported MIDI instrument combinations.
  function fitBox(box, yaw, pitch, margin = 1.16, focused = false) {
    if (!camera) return null;
    const size = box.getSize(new T.Vector3());
    const target = box.getCenter(new T.Vector3());
    // Lift the band slightly above screen centre so the bottom transport does not sit
    // on top of the front line. Keep focused objects essentially centred.
    if (!focused) target.y -= size.y * .055;
    ...
    for (const point of boxCorners(box)) {
      const v = point.sub(target);
      const depthOffset = v.dot(forward);
      distance = Math.max(distance,
        Math.abs(v.dot(right)) * margin / tanH - depthOffset,
        Math.abs(v.dot(up)) * margin / tanV - depthOffset
      );
    }
  ...
  function setDesired(target, yaw, pitch, distance, instant = false) {
    ...
    if (instant || reducedMotion) {
      state.target.copy(want.target);
      state.yaw = want.yaw;
      state.pitch = want.pitch;
      state.distance = want.distance;
      applyPose();
      renderNow();
      return;
    }
  ...
  function frame(now) {
    ...
    renderNow();
    const moving = state.target.distanceTo(want.target) +
      Math.abs(state.yaw-want.yaw) + Math.abs(state.pitch-want.pitch) +
      Math.abs(state.distance-want.distance) > .0015;
  ...
  function updatePresetUi() {
    ...
    if (note) {
      note.textContent = focusedRoot
        ? `聚焦 · ${rootLabel(focusedRoot)} · 双击空地返回全景`
        : '双击乐器聚焦 · 左拖旋转 · 右拖平移 · 滚轮缩放';
    }
  }
  ...
  function goView(id = 'front', instant = false) {
    ...
    focusedRoot = null;
    activeView = id;
    const fit = fitBox(box, spec.yaw, spec.pitch, spec.margin, false);
    if (!fit) return false;
    baseDistance = fit.distance;
    setDesired(fit.target, spec.yaw, spec.pitch, fit.distance, instant);
    updatePresetUi();
    return true;
  }
  ...
  function focusRoot(root, instant = false) {
    ...
    // A mild three-quarter bias gives guitars/drums readable volume without turning
    // focus into a side view. Keyboard keeps a nearly audience-facing presentation.
    let yaw = .05;
    if (rootLabel(root).includes('架子鼓')) yaw = .16;
    else if (rootLabel(root).includes('电吉他')) yaw = .10;
    else if (rootLabel(root).includes('木吉他')) yaw = -.08;
    else if (rootLabel(root) === 'Bass') yaw = .08;
    const pitch = rootLabel(root).includes('双层键盘') ? .18 : .20;
    const fit = fitBox(box, yaw, pitch, 1.28, true);
    if (!fit) return false;
    // Preserve exact model centre for focus; fitBox only applies stage offset when not focused.
    fit.target.copy(center);
    focusedRoot = root;
    activeView = 'focus';
    baseDistance = fit.distance;
    setDesired(fit.target, yaw, pitch, fit.distance, instant);
    updatePresetUi();
    return true;
  }
  ...
  function zoomLimits() {
    return focusedRoot
      ? [Math.max(.9, baseDistance*.30), Math.max(6, baseDistance*2.8)]
      : [Math.max(5, baseDistance*.54), Math.max(18, baseDistance*2.5)];
  }

  function zoomBy(factor) {
    const [min,max] = zoomLimits();
    const d = T.MathUtils.clamp(state.distance * factor, min, max);
    state.distance = want.distance = d;
    activeView = focusedRoot ? 'focus' : 'custom';
    updatePresetUi();
    renderNow();
  }

  function orbit(dx, dy) {
    state.yaw = want.yaw = state.yaw - dx * .0062;
    state.pitch = want.pitch = T.MathUtils.clamp(state.pitch - dy * .0046, .10, 1.22);
    activeView = focusedRoot ? 'focus' : 'custom';
    updatePresetUi();
    renderNow();
  }
  ...
  function pan(dx, dy) {
    ...
    const worldPerPixel = state.distance * 2 * Math.tan(T.MathUtils.degToRad(camera.fov/2)) / height;
    camera.updateMatrixWorld(true);
  ...
  function bandRootFor(object) {
    for (let o = object; o; o = o.parent) if (roots.includes(o)) return o;
    return null;
  }

  function playableHit(clientX, clientY) {
    const hit = intersectionsAt(clientX, clientY)[0]?.object;
  ...
  function pickedRoot(clientX, clientY) {
    for (const hit of intersectionsAt(clientX, clientY)) {
      const root = bandRootFor(hit.object);
      if (root) return root;
    }
    return null;
  }
  ...
  function setupCanvas() {
    ...
    if (!canvas || canvas.dataset.cameraV2 === '1') return;
    canvas.dataset.cameraV2 = '1';

    canvas.addEventListener('contextmenu', capture, true);
    canvas.addEventListener('pointerdown', event => {
      // Preserve the keyboard's physical key/pedal pointer interaction.
      if (event.button === 0 && !event.shiftKey && playableHit(event.clientX,event.clientY)) return;
      capture(event);
      const pointer = { id:event.pointerId, x:event.clientX, y:event.clientY, startX:event.clientX, startY:event.clientY, type:event.pointerType, button:event.button, shift:event.shiftKey };
    ...
    canvas.addEventListener('pointermove', event => {
      ...
      if (pointers.size >= 2) {
        ...
        gesture = { cx, cy, dist };
      } else if (pointer.button === 2 || pointer.shift) {
        pan(dx, dy);
      } else {
        orbit(dx, dy);
      }
    ...
    const endPointer = event => {
      ...
      // Touch double-tap mirrors desktop double-click focus.
    ...
    canvas.addEventListener('wheel', event => {
      capture(event);
      zoomBy(Math.exp(T.MathUtils.clamp(event.deltaY,-180,180)*.00155));
  ...
  function setupUi() {
    ...
    if (!menu || menu.dataset.cameraV2 === '1') return;
    menu.dataset.cameraV2 = '1';
    ...
    if (controls) controls.innerHTML = `
      ...
      <button class="view" data-camera="top" aria-pressed="false">俯视</button>`;
    const tools = menu.querySelector('.right-tools');
    if (tools) tools.innerHTML = `
      <button class="circle" id="zoom-in" type="button" title="放大" aria-label="放大">＋</button>
      <button class="circle" id="zoom-out" type="button" title="缩小" aria-label="缩小">−</button>
      <button class="circle" id="reset" type="button" title="回到全乐队" aria-label="回到全乐队">⌂ 全景</button>`;
    const note = document.createElement('div');
    note.id = 'camera-v2-note';
    note.className = 'camera-v2-note';
    note.textContent = '双击乐器聚焦 · 左拖旋转 · 右拖平移 · 滚轮缩放';
    menu.querySelector('.camera-panel')?.appendChild(note);

    const style = document.createElement('style');
    style.textContent = `
      .camera-panel{width:214px!important}
    ...
    menu.querySelectorAll('[data-camera]').forEach(button => {
      button.addEventListener('click', event => {
        capture(event);
        goView(button.dataset.camera);
        stage?.focus({preventScroll:true});
      }, true);
    });
    document.getElementById('zoom-in')?.addEventListener('click', event => { capture(event); zoomBy(.82); }, true);
    document.getElementById('zoom-out')?.addEventListener('click', event => { capture(event); zoomBy(1/.82); }, true);
    document.getElementById('reset')?.addEventListener('click', event => { capture(event); goView('front'); }, true);
  }

  let refitTicket = 0;
  function scheduleRefit() {
    const ticket = ++refitTicket;
    requestAnimationFrame(() => requestAnimationFrame(() => {
      if (ticket !== refitTicket || !ready) return;
      goView('front');
    }));
  }
  ...
  for (const id of ['bp-song','bp-mode','bp-instrument','pr-target']) {
    document.getElementById(id)?.addEventListener('change', scheduleRefit);
  }
  window.addEventListener('resize', () => {
    if (!ready) return;
    requestAnimationFrame(() => {
      if (focusedRoot) focusRoot(focusedRoot, true);
      else if (VIEWS[activeView]) goView(activeView, true);
      else renderNow();
    });
  });
  ...
  Promise.resolve(window.stageRigReady).then(() => {
    setupCanvas();
    ready = true;
    // The legacy app has finished laying out the current song by this point. Fit from
    // actual visible model bounds rather than any authored width/height constants.
    goView('front', true);
  }).catch(() => {});
