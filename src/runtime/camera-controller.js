...
// Camera v2 · phase 2.
// Box3/FOV fitting remains the safety frame. Phase 2 adds instrument-specific local
// compositions plus user camera preferences without touching MIDI/audio controllers.
(() => {
  const T = THREE;
  ...
  const VIEWS = {
    front: { yaw: 0.10, pitch: 0.13, fov: 30, margin: 1.08, distanceScale: 1.08, bias:{x:.01,y:-.12,z:.08} },
    left:  { yaw:-0.62, pitch: 0.16, fov: 31, margin: 1.07, distanceScale: 1.06, bias:{x:-.055,y:-.10,z:.035} },
    right: { yaw: 0.62, pitch: 0.16, fov: 31, margin: 1.07, distanceScale: 1.06, bias:{x:.055,y:-.10,z:.035} },
    top:   { yaw: 0.13, pitch: 0.62, fov: 33, margin: 1.08, distanceScale: 1.08, bias:{x:0,y:-.045,z:.02} },
  };
  ...
  // Regions are normalized fractions of the instrument's world-space Box3.
  // This deliberately avoids hard-coding model internals, so the local views survive
  // later procedural-model revisions and song-mode 180° keyboard rotation.
  ...
  function regionBox(box, region){
    ...
    for(const axis of ['x','y','z']){
      ...
      if(!range)continue;
      const min=box.min[axis]+size[axis]*T.MathUtils.clamp(range[0],0,1);
      const max=box.min[axis]+size[axis]*T.MathUtils.clamp(range[1],0,1);
      out.min[axis]=Math.min(min,max);
      out.max[axis]=Math.max(min,max);
    }
    return out;
  }
  ...
  function fitBox(box,yaw,pitch,fov,margin=1.08,targetOverride=null,focused=false){
    ...
    distance=Math.max(distance,diagonal*(focused?.48:.38),focused?1.4:8.5);
  ...
  function frame(now){
    ...
    renderNow();
    const moving=state.target.distanceTo(want.target)+Math.abs(state.yaw-want.yaw)+
      Math.abs(state.pitch-want.pitch)+Math.abs(state.distance-want.distance)+
      Math.abs(state.fov-want.fov)*.02>.0015;
  ...
  function setDesired(target,yaw,pitch,distance,fov=30,instant=false){
    ...
    if(instant||reducedMotion){
      ...
      state.distance=want.distance;state.fov=want.fov;renderNow();
    }else scheduleFrame();
  }
  ...
  function renderFocusUi(){
    ...
    menu.classList.toggle('instrument-focus',focused);
    ...
    for(const item of currentInstrumentViews()){
      ...
      button.type='button';
      button.dataset.focusView=item.id;
      button.textContent=item.label;
      button.setAttribute('aria-pressed',String(item.id===focusedView));
      button.addEventListener('click',event=>{
        stop(event);
        focusView(focusedRoot,item.id);
  ...
  function updateUi(){
    ...
    if(note)note.textContent=focusedRoot
      ? `${rootLabel(focusedRoot)} · 选择局部机位 · 双击空地返回乐队`
  ...
  function focusView(root,viewId='overall',instant=false){
    ...
    const box=regionBox(whole,view.region);
    const spec=view.spec||FOCUS[type]||FOCUS.acoustic;
    const target=composedTarget(box,spec);
  ...
  function orbit(dx,dy){
    ...
    state.pitch=want.pitch=T.MathUtils.clamp(
      state.pitch-dy*.0042*sensitivity*verticalSign,.08,1.12
    );
  ...
  function pan(dx,dy){
    ...
    const worldPerPixel=state.distance*2*Math.tan(T.MathUtils.degToRad(camera.fov/2))/
  ...
  function setupUi(){
    ...
    focusSection.className='camera-focus-section';
    ...
    prefs.className='camera-pref-section';
    prefs.innerHTML=
    ...
    note.id='camera-v2-note';note.className='camera-v2-note';
    note.textContent='摄影机位 · 双击乐器聚焦 · 左拖旋转 · 右拖平移';
    panel?.appendChild(note);

    if(!document.getElementById('camera-v2-style')){
      const style=document.createElement('style');
      style.id='camera-v2-style';
      style.textContent=
        '.camera-panel{width:238px!important}'+
        ...
        '.camera-subviews{display:grid;grid-template-columns:1fr 1fr;gap:5px}.camera-subviews .view{height:32px}'+
    ...
    if(vertical){
      ...
      vertical.addEventListener('change',()=>{
        cameraPrefs.vertical=vertical.value==='inverted'?'inverted':'standard';
        savePrefs();
      });
    }
    if(sensitivity){
      ...
      sensitivity.addEventListener('change',()=>{
        cameraPrefs.sensitivity=SENSITIVITY[sensitivity.value]?sensitivity.value:'medium';
        savePrefs();
      });
    }
  }
    ...
    window.VirtualBandCamera={
      ...
      get state(){
        return {
          ...
          target:state.target.clone(),preferences:{...cameraPrefs}
        };
      }
    };
    console.info('[Camera v2] phase 2 instrument views attached');
    return true;
  }
