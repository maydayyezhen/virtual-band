...
export class AtelierElectricShowcaseMode implements PresentationMode {
  ...
  private readonly onPointerDown = (event: PointerEvent): void => {
    ...
    const hit = event.button === 0 && !event.shiftKey
      ? this.interactions.hitTest(event.clientX, event.clientY, this.electric.id)
      : null;
  ...
  private readonly onKeyDown = (event: KeyboardEvent): void => {
    ...
    if (action?.kind === 'string') {
      if (!event.repeat && !this.computerKeys.has(event.code)) {
        const result = this.electric.pluck(action.stringNumber, 104, 0);
        if (result) {
  ...
  private readonly onKeyUp = (event: KeyboardEvent): void => {
    ...
    if (!held) return;
    this.electric.noteOff(held.note);
  ...
  private releaseInputState(): void {
    ...
    for (const held of this.computerKeys.values()) this.electric.noteOff(held.note);
    this.computerKeys.clear();
    this.pointers.clear();
    this.previousGesture = null;
    this.element.classList.remove('dragging');
