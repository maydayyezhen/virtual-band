...
export class AtelierAcousticShowcaseMode implements PresentationMode {
  ...
  private getPreset(id: AtelierAcousticViewName): InstrumentOrbitCameraView | null {
    const view = this.cameraRegistry.get(ATELIER_ACOUSTIC_VIEW_IDS[id]);
  ...
  private readonly onKeyDown = (event: KeyboardEvent): void => {
    ...
    if (action?.kind === 'string') {
      ...
      this.want.pitch = THREE.MathUtils.clamp(this.want.pitch + 0.08, -0.92, 1.08);
      handled = true;
  ...
  private changeZoom(factor: number): void {
    ...
    if (!preset) return;
    this.zoom = THREE.MathUtils.clamp(this.zoom * factor, 0.28, 1.7);
    this.want.distance = this.distanceFor(preset) * this.zoom;
  }
  ...
  private pan(dx: number, dy: number): void {
    ...
    this.acoustic.root.updateWorldMatrix(true, true);
    const worldTarget = this.acoustic.root.localToWorld(this.current.target.clone());
    ...
    this.want.target.copy(this.acoustic.root.worldToLocal(movedWorldTarget));
  }
  ...
  private applyCamera(): void {
    ...
    const position = this.acoustic.root.localToWorld(localPosition);
    const target = this.acoustic.root.localToWorld(this.current.target.clone());
