...
可以按目标屏幕手动导入图片、视频或换程序动画；「恢复歌曲画面」让所选屏幕追上当前歌曲时间。「恢复默认」回到场馆默认图案。换成未匹配歌曲或布局时，`SongScreens` 只释放仍属于旧歌曲内容对象的屏幕，保留手动替换的素材；重新载入匹配示例则重新启用三屏编排。关闭歌曲灯光不影响歌曲画面。作品导播已接入统一歌曲时间，参见 [演出创作接口](SHOW_AUTHORING.md)；通用素材时间线编辑器尚未实现。
