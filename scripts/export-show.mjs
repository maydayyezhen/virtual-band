...
import { loadCatalog, checkAssets } from './lib/show-catalog.mjs';
...
if (argv.includes('--list')) {
  console.log(catalog.SHOW_EXAMPLES.map(s => `${s.id} | ${s.title} | ${s.offline.supported ? 'offline' : s.offline.reason}`).join('\n'));
  process.exit(0);
}
const show = catalog.getShow(arg('song', 'bohemian-rhapsody'));
if (!show.offline.supported) throw new Error(`${show.id} 暂不支持离线导出：${show.offline.reason}`);
const { midiPath } = await checkAssets(show);
...
const output = resolve(root, arg('output', `exports/${show.id}-1080p${fps}.mp4`));
...
const work = await fs.mkdtemp(join(dirname(output), '.show-export-'));
...
try {
  ...
  const loaded = await page.evaluate(id => window.bandView.loadLightingExample(id), show.id);
  assert.equal(loaded, true, `作品 ${show.id} 加载失败`);
  const metadata = await page.evaluate(async () => {
    await document.fonts.ready;
    const v = window.bandView;
    if (!v.player || !v.cameraShow.available) throw Error('所选作品未加载成功或缺少导播轨道');
  ...
  const audio = await renderMidiAudio({ midiPath,
  ...
  await run('ffmpeg', ['-hide_banner', '-loglevel', 'warning', '-r', String(fps), '-i', video, '-i', wav,
    ...
    '-movflags', '+faststart', '-metadata', `title=${show.menu?.title ?? show.title}`, '-metadata', `artist=${show.artist}`, '-metadata', `comment=${show.credit}`, partial]);
  ...
  const report = { song: show.id, midiSha256: show.sha256, output, width, height, fps, frames, duration: videoDuration, bytes: (await fs.stat(output)).size,
  ...
  try { await fs.access(output); assert.equal(dirname(resolve(work)), dirname(output)); await fs.rm(work, { recursive: true }); }
